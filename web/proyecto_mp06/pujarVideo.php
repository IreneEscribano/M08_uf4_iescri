...
if($nom == "" || $descripcio == "" || $ubicacio == "") {
    ...
    include("connect.php");
    $sql = "INSERT INTO tb_videos (id_artista, nom, descripcio, data, hora, ubicacio, adult) VALUES ($id_usuari, '$nom', '$descripcio', curdate(), curtime(), '$ubicacio', $adult)";
    $result = mysqli_query($con,$sql) or exit(mysqli_error($con).$sql);
    echo !mysqli_error($con) ? "Vídeo pujat" : "Error al pujar el vídeo";
}
?>